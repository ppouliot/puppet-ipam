  ...
  describe 'ipam::install' do
    ...
    on_supported_os.reject { |_, f| f[:os]['family'] == 'Solaris' }.each do |os, f|
      context "on #{os}" do
        ...
        it {
          is_expected.to contain_class('staging')
            .with(
              path: '/opt/staging', 
              owner: 'root',
              group: 'root',
        ...
        it {
          is_expected.to contain_staging__deploy('dhcpd-pool-0.2.tar.gz')
            .with(
        ...
        it {
          is_expected.to contain_staging__file('dhcpd-pools-2.29.tar.xz')
            .with(
        ...
        it {
          is_expected.to contain_staging__deploy('dhcpstatus_0.60.tar.gz')
            .with(
